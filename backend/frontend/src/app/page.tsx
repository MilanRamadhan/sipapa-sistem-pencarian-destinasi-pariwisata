...
export default async function HomePage() {
  ...
  if (secondary) {
    ...
    if (kepadaIndex !== -1) {
      secondary.snippet =
        secondary.snippet.substring(0, kepadaIndex + 6) + "...";
    }
  }

  const quickFilters = [
    { label: "Destinasi", query: "destinasi wisata" },
    { label: "Pantai", query: "wisata pantai" },
    { label: "Gunung", query: "wisata gunung" },
    { label: "Kuliner", query: "wisata kuliner" },
    { label: "Budaya & Festival", query: "budaya dan festival" },
    { label: "Semua", query: "wisata indonesia" },
  ];
    ...
    <main className="min-h-screen px-4 py-6">
      ...
      <header className="max-w-6xl mx-auto border-b border-neutral-300 pb-4 mb-6">
        <div className="flex items-center justify-between mb-3 gap-4">
          <button className="text-xs tracking-[0.2em] uppercase text-neutral-500">
            SIPAPA DAILY
          </button>

          <h1
            className="heading-serif text-4xl md:text-5xl font-black text-center flex-1"
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          {hero && (
            <article className="md:col-span-2 border-b md:border-b-0 md:border-r border-neutral-300 pr-0 md:pr-6 pb-6 md:pb-0">
              ...
              <a
                href={`/detail/${hero.doc_id}`}
                className="cursor-pointer block hover:opacity-80 transition-opacity"
              >
                <h1 className="heading-serif text-2xl md:text-3xl font-bold mb-3 leading-tight">
                  {hero.title}
                </h1>
              </a>
              <p className="text-sm text-neutral-700 leading-relaxed mb-3 line-clamp-4">
                {hero.snippet}
              </p>
              <a
                href={`/detail/${hero.doc_id}`}
                className="text-xs font-semibold tracking-[0.15em] uppercase text-blue-700 hover:underline"
              >
                BACA SELENGKAPNYA →
              </a>
              {hero.image_url && (
          ...
          {secondary && (
            <article className="bg-black text-white p-4 rounded-md flex flex-col min-h-[350px] md:min-h-[420px]">
              <p className="text-[9px] uppercase tracking-[0.25em] text-neutral-400 mb-1">
                SPOT REKOMENDASI
              </p>
              <a
                href={`/detail/${secondary.doc_id}`}
                className="cursor-pointer block hover:opacity-90 transition-opacity"
              >
                <h2 className="heading-serif text-lg md:text-xl font-bold mb-2 leading-tight">
                  {secondary.title}
                </h2>
              </a>
              <p className="text-sm text-neutral-300 leading-[1.5] flex-1">
                {secondary.snippet}
              </p>
              <a
                href={`/detail/${secondary.doc_id}`}
                className="text-[10px] font-semibold tracking-wide uppercase text-white hover:text-gray-300 transition-colors mt-2"
              >
                JELAJAHI DESTINASI →
              </a>
            </article>
          ...
          <div className="grid gap-4 md:grid-cols-5">
            {others.map((art) => (
              <article key={art.doc_id} className="group pb-3">
                ...
                <a
                  href={`/detail/${art.doc_id}`}
                  className="cursor-pointer block"
                >
                  <h3 className="heading-serif text-sm font-bold leading-snug mb-2 line-clamp-3 group-hover:text-blue-700 transition-colors">
                    {art.title}
                  </h3>
                </a>
                ...
                <a
                  href={`/detail/${art.doc_id}`}
                  className="text-[11px] uppercase tracking-[0.16em] text-blue-700 hover:underline font-semibold"
                >
                  BACA →
                </a>
              </article>
